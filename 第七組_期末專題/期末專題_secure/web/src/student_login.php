...
if ($_POST) {
    ...
    */
    ...
    $sql = "SELECT * FROM userTable  
            JOIN scoreTable ON userTable.username = scoreTable.studentId  
            WHERE userTable.username = ? AND userTable.password = ?";
    // 2. 使用 prepare 代替直接執行
    ...
    if (!$result) {
        ...
        if ($result->num_rows === 0) {
            ...
            $_SESSION['student_sql_rows'] = $rows;
    ...
    if ($conn->multi_query($sql)) {
        ...
        elseif ($result->num_rows === 0) {
           ...
           // 取得並存下查詢結果（把 DB 撈出的資料存進 Session，結果頁只讀 Session）
            ...
            $_SESSION['student_sql_rows'] = $rows; // 存成陣列（可能是一筆或多筆，方便示範時顯示）
...
<html lang="zh-TW">
<head>
    <meta charset="UTF-8"> <title>(安全版)學生成績查詢系統</title> <style>
